...
const Skills = () => {
  ...
  const handlePrevious = () => {
    if (currentIndex > 0) {
      ...
      setVisibleSkills(prevVisible => {
        ...
        newVisible.pop(); // Remove the last CircularProgressbar
        newVisible.unshift(SkillsData[currentIndex - 1]); // Add the new CircularProgressbar at the beginning
        return newVisible;
      });
    }
  };
  ...
  const handleNext = () => {
    if (currentIndex < SkillsData.length - itemsPerSlide) {
      setCurrentIndex(currentIndex + 1);
      setVisibleSkills(prevVisible => [
        ...prevVisible.slice(1), // Remove the first CircularProgressbar
        SkillsData[currentIndex + itemsPerSlide], // Add the new CircularProgressbar at the end
      ]);
    }
  };
